...
import koreatech.cse.domain.myapi.location.MyNearestPark;
import koreatech.cse.domain.myapi.parkinfo.MyParkInfo;
import koreatech.cse.domain.myapi.station.MyNearestStation;
                ...
                for(int j =1 ; j < 4 ; j++)
                {
                    ...
                    if(i == 2)
                    {
                        ...
                        ResponseEntity<TrainStation> trainStation2ResponseEntity
                                = T2restTemplate.getForEntity("https://apis.daum.net/local/v1/search/keyword.json?apikey=f72e008bdadbf7dddd402aebdedc461e&" +
    ...
    public  ResponseEntity< NearestPark> getParkWithJustAPIKey(Model model,
        ...
        try {
            ...
            ResponseEntity<CurrentLocation> currentLocResponseEntity
                    = CurrentLocTemplate.getForEntity("https://apis.daum.net/local/geo/addr2coord?apikey=f72e008bdadbf7dddd402aebdedc461e&q=" + address +"&output=json", CurrentLocation.class);
   ...
   // public String getviewWithJustAPIKey(Model model,@RequestParam(name = "address") String address,
       ...
       //1
        ...
        try {
            //서울공원
            ResponseEntity<SeoulPark> parkResponseEntity
                    = PrestTemplate2.getForEntity("http://openapi.seoul.go.kr:8088/664b6b4962646c61313234774c646550/json/SearchParkInfoService/1/97/", SeoulPark.class);
        ...
        List<Child> childlist1 = new ArrayList<Child>();
            ...
            for( String key : myPark.getPark().keySet() ){
                ...
                child1.setName(key +"..."+ myPark.getPark().get(key) + "m");
                //child1.setChildren();
                ...
                for(Output row : list)
                {
                    System.out.println(row.getPname()+ ","+key);
                    if(row.getPname()==key)
                    {
                        Child_ child2 = new Child_();
                        child2.setName(row.getBcount()+"");
                        childlist2.add(child2);

                        Child_ child2_1 = new Child_();
                        child2_1.setName(row.getRcount()+"");
